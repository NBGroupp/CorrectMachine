...
TRAIN_EPOCH_SIZE = int(TRAIN_DATA_SIZE / TRAIN_BATCH_SIZE +0.5)
TEST_EPOCH_SIZE = int(TEST_DATA_SIZE / TEST_BATCH_SIZE + 0.5)
...
class Proofreading_Model(object):
    def __init__(self, is_training, batch_size, num_steps):
        ...
        #全连接层
        # dense_input = (output1 + output2)
        # self.output=tf.layers.dense(inputs=dense_input, units=HIDDEN_SIZE, activation=tf.nn.relu)
        self.output=(output1+output2)/2
        # print(self.output)
        # self.output = tf.div(tf.add(output1, output2), 2)
        #embedding长度转化为词典长度
        #self.output = tf.reshape(tf.concat(self.output, 1), [-1, HIDDEN_SIZE]) #output=[batch_size*num_steps,hidden_size]
...
def main():
    ...
    with tf.Session() as session:
        ...
        saver.save(session, "../ckpt2/model.ckpt")
